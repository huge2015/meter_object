...
class ModUploadControlHelper
{
	public function getElectricalData($start_time, $end_time) {
		// read electrical status
		$db = JFactory::getDbo();
		$query = "SELECT * FROM #__electrical WHERE datetime >= '$start_time' AND datetime <= '$end_time' ORDER BY electrical_id ASC";
...
//		$electrical_id = $row['electrical_status'];
        if($rows == ""){ //while the electrical table has delete or anyway trouble  but the time will update
		    ...
		    $rows = $db->loadAssocList();
			*/
			echo " <script>alert('数据库中不存在此条件记录，请检查填入数据是否有误！');history.back(); </script>";
		}
		return $rows;
	}
	...
	public function insertQueueId($electrical_id, $location_id, $meter_address, $datetime) {
		
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query = "SELECT * FROM joomla3_queue WHERE electrical_id = '$electrical_id'";
        ...
        if($row == ""){
		    ...
		    // Create and populate an object.
		    $profile = new stdClass();
		    $profile->electrical_id = $electrical_id;
		    ...
		    $result = JFactory::getDbo()->insertObject('joomla3_queue', $profile);
		}else{
