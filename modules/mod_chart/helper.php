...
//JLog::add(JText::_("jtime datetime is : ".$jtime), JLog::ERROR, 'jerror');	 
        ...
        for ($s = 0; $s < $mchk; $s++){
			...
			for($m = 0; $m < $count_max; $m++){
				if(sizeof($meter_address) == 1){
					...
					$jrows[$m]['datetime'] = $format_datetime[$MaxMeterId.'_'.$m];
					$jrows[$m]['Meter'.$meter_address[$s].'_phase1'] = $phase1[$s.'_'.$m];
					  //if($jrows[$m]['Meter'.$meter_address[$s].'_phase1'] == ''){$jrows[$m]['Meter'.$meter_address[$s].'_phase1'] = '0.0000';}
					$jrows[$m]['Meter'.$meter_address[$s].'_phase2'] = $phase2[$s.'_'.$m];
					  //if($jrows[$m]['Meter'.$meter_address[$s].'_phase2'] == ''){$jrows[$m]['Meter'.$meter_address[$s].'_phase2'] = '0.0000';}
					$jrows[$m]['Meter'.$meter_address[$s].'_phase3'] = $phase3[$s.'_'.$m];
					  //if($jrows[$m]['Meter'.$meter_address[$s].'_phase3'] == ''){$jrows[$m]['Meter'.$meter_address[$s].'_phase3'] = '0.0000';}
					
					for($i = 1; $i < $mchk; $i++){
						$jrows[$m]['Meter'.$meter_address[$i].'_phase1'] = $phase1[$i.'_'.$m];
						  //if($jrows[$m]['Meter'.$meter_address[$i].'_phase1'] == ''){$jrows[$m]['Meter'.$meter_address[$i].'_phase1'] = '0.0000';}
					    $jrows[$m]['Meter'.$meter_address[$i].'_phase2'] = $phase2[$i.'_'.$m];
						  //if($jrows[$m]['Meter'.$meter_address[$i].'_phase2'] == ''){$jrows[$m]['Meter'.$meter_address[$i].'_phase2'] = '0.0000';}
					    $jrows[$m]['Meter'.$meter_address[$i].'_phase3'] = $phase3[$i.'_'.$m];
                          //if($jrows[$m]['Meter'.$meter_address[$i].'_phase3'] == ''){$jrows[$m]['Meter'.$meter_address[$i].'_phase3'] = '0.0000';}						
					}
	...
	public static function getTChartDataAjax(){
		// $table is the table to read from. can have electrical, water, etc
		// $location_id is the location id. unique for each building
		// $meter_address is an array of the meter_address to read pertaining to the location
		// $from_datetime is the datetime to start the search from
...
//JLog::add(JText::_("into modTchartHelper  !"), JLog::ERROR, 'jerror');		
/*
echo "meter_address is $meter_address ---";
echo "columns_string is $columns_string ---";
echo "num_records is $num_records ---";
...
*/
		...
		if ($columns_string == null) {
			if ($t == 1) {
				$columns = array('electrical_id', 'meter_address', 'location_id', 'datetime', 'total_power_apparent_power', 'total_power_voltage', 'total_power_current', 'total_power_frequency');
				$select_string = '`electrical_id`, `meter_address`, `location_id`, `datetime`, `total_power_apparent_power`, `total_power_voltage`, `total_power_current`, `total_power_frequency`';
			} else {
				$columns = array('electrical_id', 'meter_address', 'location_id', 'MAX(`datetime`)', 'AVG(total_power_apparent_power)', 'AVG(total_power_voltage)', 'AVG(total_power_current)', 'AVG(total_power_frequency)');
				$select_string = '`electrical_id`, `meter_address`, `location_id`, MAX(`datetime`) AS `datetime`, AVG(`total_power_apparent_power`) AS total_power_apparent_power, AVG(`total_power_voltage`) AS total_power_voltage, AVG(`total_power_current`) AS total_power_voltage, AVG(`total_power_frequency`) AS total_power_frequency';
			} // if t = 1
		} else {
        */
			$columns = explode(',', $columns_string);
...
JLog::add(JText::_("MaxRecord  is : ".$MaxRecord), JLog::ERROR, 'jerror');	
JLog::add(JText::_("MaxMeterId  is : ".$MaxMeterId), JLog::ERROR, 'jerror');		
*/	
...
//  echo("query is " . $query->__toString() . '---');
		...
		foreach ($rows as $row){
			//for($cols = 1; $cols < sizeof($columns); $col++){
				//$total_power[$s."_".$m] = $row["$columns[1]"];
			//}
			$total_power[$s."_".$m] = $row["$columns[1]"];
			$format_datetime[$s."_".$m] = $row['datetime'];
...
//JLog::add(JText::_("jtime datetime is : ".$jtime), JLog::ERROR, 'jerror');	 
        ...
        for ($s = 0; $s < $mchk; $s++){
			...
			for($m = 0; $m < $count_max; $m++){
				if(sizeof($meter_address) == 1){
					...
					$jrows[$m]['Meter'.$meter_address[$s].'_total_power'] = $total_power[$s.'_'.$m];
					
				}else{
					
					$jrows[$m]['datetime'] = $format_datetime[$s.'_'.$m];
					  if($jrows[$m]['datetime'] == ''){$jrows[$m]['datetime'] = $format_datetime[$MaxMeterId.'_'.$m];}
					$jrows[$m]['Meter'.$meter_address[$s].'_total_power'] = $total_power[$s.'_'.$m];
					  if($jrows[$m]['Meter'.$meter_address[$s].'_total_power'] == ''){$jrows[$m]['Meter'.$meter_address[$s].'_total_power'] = "0.00000000";}
					
					for($i = 1; $i < $mchk; $i++){
						$jrows[$m]['Meter'.$meter_address[$i].'_total_power'] = $total_power[$i.'_'.$m];
						  if($jrows[$m]['Meter'.$meter_address[$i].'_total_power'] == ''){$jrows[$m]['Meter'.$meter_address[$i].'_total_power'] = "0.00000000";}						
					}
        ...
        $json_rows = json_encode($jrows);
//JLog::add(JText::_("json_rows  is : ".$json_rows), JLog::ERROR, 'jerror');			
		return $json_rows;
	} // getTChartData
	
	
	
	public static function getEChartDataAjax(){
		// $table is the table to read from. can have electrical, water, etc
		// $location_id is the location id. unique for each building
		// $meter_address is an array of the meter_address to read pertaining to the location
		// $from_datetime is the datetime to start the search from
...
//JLog::add(JText::_("jtime datetime is : ".$jtime), JLog::ERROR, 'jerror');	 
        ...
        for ($s = 0; $s < $mchk; $s++){
			...
			for($m = 0; $m < $count_max; $m++){
				if(sizeof($meter_address) == 1){
		            $jrows[$m]['datetime'] = $format_datetime[$s.'_'.$m];
					$jrows[$m]['Meter'.$meter_address[$s].'_energy'] = $energy[$s.'_'.$m];
					...
					$jrows[$m]['datetime'] = $format_datetime[$s.'_'.$m];
					  if($jrows[$m]['datetime'] == ''){$jrows[$m]['datetime'] = $format_datetime[$MaxMeterId.'_'.$m];}
					$jrows[$m]['Meter'.$meter_address[$s].'_energy'] = $energy[$s.'_'.$m];
					  if($jrows[$m]['Meter'.$meter_address[$s].'_energy'] == ''){$jrows[$m]['Meter'.$meter_address[$s].'_energy'] = "0.00000000";}
					
					for($i = 1; $i < $mchk; $i++){
						$jrows[$m]['Meter'.$meter_address[$i].'_energy'] = $energy[$i.'_'.$m];
						  if($jrows[$m]['Meter'.$meter_address[$i].'_energy'] == ''){$jrows[$m]['Meter'.$meter_address[$i].'_energy'] = "0.00000000";}						
					}
