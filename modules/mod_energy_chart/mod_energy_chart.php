...
$js = JURI::base().'modules/mod_energy_chart/js/jsapi.js';  
$document = JFactory::getDocument($js);  
$document->addScript($js); 

$zh_CN = JURI::base().'modules/mod_energy_chart/js/format+zh_CN,default+zh_CN,ui+zh_CN,corechart+zh_CN.I.js';  
$document2 = JFactory::getDocument($zh_CN);  
$document2->addScript($zh_CN); 

$corechart = JURI::base().'modules/mod_energy_chart/js/corechart.js';  
$document3 = JFactory::getDocument($corechart);  
$document3->addScript($corechart);



$date = JURI::base().'modules/mod_energy_chart/js/date.js';  
$document4 = JFactory::getDocument($date);  
$document4->addScript($date); 

$jq = JURI::base().'modules/mod_energy_chart/js/jquery-1.9.1.js';  
...
<script type="text/javascript">// <![CDATA[
	...
	chartCol[1] = {'db_col_name': 'energy_kwh', 'chart_data_type' : 'number', 'chart_label' : 'Energy'};
	...
	for( var m = 0; m < mchk; m++){
        ...
        var Ms = new Array(); 
			...
			jsonCol[j] =  {'db_col_name':'Meter'+Ms[m]+'_energy','chart_data_type' : 'number', 'chart_label':'Meter'+Ms[m]+'_energy'};
			j++;
   ...
   for (var jk = 0; jk < jsonCol.length; jk++){
	...
	function drawChart() {
	    ...
	    for( var m = 0; m < mchk; m++){
             ...
             var Ms= new Array(); 
                 Ms=Mname.split("-"); 
		    chartData.addColumn('number', 'Meter'+ Ms[m] +'_energy');
	    }

		// Set chart options
		chartOptions = {'title':'Energy against Time (Last updated time is ' + Date.now().toString('yyyy-MM-dd HH:mm:ss')+ ')  Meter Selected: [ '+ meter_select +' ]',
			'vAxis':{'title':'Energy (kWh)', 'minValue':vChartAxisMinValue, 'maxValue':vChartAxisMaxValue},
...
<?php /*
	echo "lastRowIndex = chartData.addRows([ ";
		...
		for($s = 0; $s < $mchk; $s++){
			...
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select( $db->quoteName(array('electrical_id', 'location_id', 'meter_address', 'datetime', 'energy') ) );
			//$query->select('*');   
			$query->from( $db->quoteName('#__electrical') );
			$query->where( 
			            $db->quoteName('location_id')." = ".$db->quote($location_id) .
        ...
        for ($s = 0; $s < $mchk; $s++){
			...
			for($t = 0; $t <$count_time[$s]; $t++){
				...
				if(sizeof($meter_address) == 1){
		            echo "[ new Date(".$format_datetime[$s."_".$t]."),".$energy[$s."_".$t]." ]";
				}else{
					echo "[ new Date(".$format_datetime[$s."_".$t]."),".$energy[$s."_".$t].", ";
...
//alert('in redrawchart()');
	...
	function getChartDataToDataToLoad(table, location_id, meter_address, columns, from_datetime, to_datetime, num_records, data_interval  )  {
		...
		jQuery.ajax({
			url: "index.php",
			data: {"option":"com_ajax", "module":"chart", "method":"getEChartData", "format":"json" ,
...
<form name=chart_form action="energy-chart" method="POST">
<table >
<tr>
<td width=500px>
...
<!--tr align="left" onmouseover="this.style.backgroundColor='#e5ff00'" onmouseout="this.style.backgroundColor='#ffffff'">
            <td style="padding-left:2px;">
			<input name="energy" id="energy" type="checkbox"  value="1"  checked ="checked" onclick="javascript:change_Voltage_A();" />&nbsp;energy&nbsp;&nbsp;
